        ...
        if (len(self._events) == 1) and (self._events[0].event_type == EventTypesRhodesmill.CULMINATES): #handles sat that is in view for entire reservation
            ...
            if len(enter_events) != len(exit_events): #Handle case where a satellite starts xor ends in observation area
                if self._events[0].event_type == EventTypesRhodesmill.ENTERS: #the first event is the satellite entering view, so it didn't start in observation area
                    end_reservation_event = EventRhodesmill(event_type=EventTypesRhodesmill.EXITS, satellite=self._events[0].satellite, timestamp=self._reservation.time.end.replace(tzinfo=pytz.UTC))
                    ...
                    start_reservation_event = EventRhodesmill(event_type=EventTypesRhodesmill.ENTERS, satellite=self._events[0].satellite, timestamp=self._reservation.time.begin.replace(tzinfo=pytz.UTC))
            ...
            time_windows = [TimeWindow(begin=begin_event.timestamp.replace(tzinfo=pytz.UTC), end=exit_event.timestamp.replace(tzinfo=pytz.UTC)) for begin_event, exit_event in enter_and_exit_pairs]
