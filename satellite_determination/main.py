...
class Main:
    ...
    def run(self) -> MainResults:
        return MainResults(
            satellites_above_horizon=self._event_finder_rhodesmill.get_overhead_windows(),
            interference_windows=self._event_finder_rhodesmill.get_overhead_windows_slew()
        )

    @cached_property
    def _event_finder_rhodesmill(self) -> EventFinderRhodesMill:
